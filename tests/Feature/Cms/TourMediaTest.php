...
class TourMediaTest extends TestCase
{
    ...
    public function the_creator_can_update_a_tours_audio()
    {
        ...
        $this->uploadMedia('intro_audio', $file, $audioFile)
            ->assertStatus(200);

        $this->assertNotEmpty($this->tour->fresh()->intro_audio);

        \Storage::disk('s3')->assertExists($file);

        $this->uploadMedia('background_audio', $file, $audioFile)
            ->assertStatus(200);
    ...
    /** @test */
    public function tour_audio_uploads_can_only_be_mp3_files()
    {
        $this->loginAs($this->business);
        ...
        $this->uploadMedia('intro_audio', $file, $pdfFile)
            ->assertStatus(422)
            ->assertSee('file of type:');

        $this->uploadMedia('background_audio', $file, $pdfFile)
            ->assertStatus(422)
            ->assertSee('file of type:');
    }

    /** @test */
    public function tour_audio_uploads_have_a_max_file_size()
    {
        $this->loginAs($this->business);
        ...
        $this->uploadMedia('intro_audio', $file, $largeFile)
            ->assertStatus(422)
            ->assertSee('may not be greater than');

        $this->uploadMedia('background_audio', $file, $largeFile)
            ->assertStatus(422)
            ->assertSee('may not be greater than');
    }
    ...
    public static $imageKeys = ['main_image', 'image_1', 'image_2', 'image_3', 'trophy_image'];
